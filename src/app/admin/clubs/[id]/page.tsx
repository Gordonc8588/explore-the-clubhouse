...
import { mockClubs, mockClubDays, mockBookingOptions } from "@/lib/mock-data";
...
export default function ClubEditPage() {
  ...
  const {
    ...
    defaultValues: {
      ...
      isActive: true,
      days: [],
      bookingOptions: isNewClub ? defaultBookingOptions : [],
    },
  });
  ...
  useEffect(() => {
    if (!isNewClub) {
      ...
      if (club) {
        ...
        setValue("imageUrl", club.image_url || "");
        setValue("startDate", club.start_date);
        setValue("endDate", club.end_date);
        setValue("morningStart", club.morning_start.slice(0, 5));
        setValue("morningEnd", club.morning_end.slice(0, 5));
        setValue("afternoonStart", club.afternoon_start.slice(0, 5));
        setValue("afternoonEnd", club.afternoon_end.slice(0, 5));
        setValue("minAge", club.min_age);
        setValue("maxAge", club.max_age);
        setValue("isActive", club.is_active);

        const clubDays = mockClubDays.filter((d) => d.club_id === clubId);
        const existingDaysMap = new Map(clubDays.map((d) => [d.date, d]));
        ...
        const options = mockBookingOptions
          .filter((o) => o.club_id === clubId)
          .map((o) => ({
            id: o.id,
            name: o.name,
        ...
        if (options.length > 0) {
          setValue("bookingOptions", options);
        }
      }
    }
  }, [clubId, isNewClub, setValue, replaceDays]);

  // Auto-generate days when dates change
  const regenerateDays = useCallback(() => {
    if (startDate && endDate) {
      const currentDays = dayFields.map((field, index) => ({
        id: field.id,
        date: field.date,
  ...
  const onSubmit = async (data: ClubFormData) => {
    ...
    try {
      ...
      console.log("Saving club data:", data);
        ...
        <div className="flex flex-col gap-4 rounded-2xl bg-white p-6 shadow-[var(--shadow-md)] sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="font-display text-2xl font-bold text-bark">
              {isNewClub ? "Create New Club" : "Edit Club"}
            </h2>
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                          ...
                          <div>
                            ...
                            <select
                              ...
                              className="w-full rounded-lg border border-stone/30 px-3 py-2 font-body text-bark transition-colors focus:border-forest focus:outline-none focus:ring-2 focus:ring-sage/30"
                            >
                              {(Object.keys(optionTypeLabels) as OptionType[]).map(
                                (type) => (
                          ...
                          <div>
                            ...
                            <select
                              ...
                              className="w-full rounded-lg border border-stone/30 px-3 py-2 font-body text-bark transition-colors focus:border-forest focus:outline-none focus:ring-2 focus:ring-sage/30"
                            >
                              {(Object.keys(timeSlotLabels) as TimeSlot[]).map(
                                (slot) => (
