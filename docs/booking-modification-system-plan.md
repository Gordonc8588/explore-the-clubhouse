...
- **Deferred (optional cleanup):** migrate `edit-days`/`reschedule` onto the shared `issueRefundAfterModification`/`createUpchargeRequest` helpers for a single source of truth (pure internal churn, no behaviour change; `reschedule` is now largely superseded by the unified editor).
