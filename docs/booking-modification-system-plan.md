...
- **Phase 6 — Unified "Edit booking" editor. ✅ BUILT.** One admin screen (`/edit` route + modal) that stages a full proposed state — target week + day set + `num_children` — and routes by signed delta, as a THIN caller of the proven core (no pricing/capacity/refund logic reimplemented). New shared helpers in `booking-modify.ts`: `computeUnifiedPricing` (cheapest valid option for any club + day count + child count; same full-week⇄per-day + snapshotted-discount rules), `issueRefundAfterModification` (canonical post-apply refund), `createUpchargeRequest` (canonical deferred payment-link flow). Money routing: delta≤0 applies the whole new state atomically via `apply_booking_modification` then refunds the difference DB-first; delta>0 uses the proven same-week payment-link flow. **Safe v1 / owner decisions:** added *alongside* the existing modals (not replacing them); cross-week or children-increase **up-charges are blocked** with a clear message, because the webhook re-prices a pending request against the booking's *current* week/children — so only same-week, same-children charges are staged (the webhook is left untouched). Plus a **Booking History** card (audit trail over `booking_modifications`). A num_children decrease never deletes child records. No new migration (rides existing tables + RPC). Self-reviewed (adversarial). _Pending: deploy._
- **Phase 7 — Full up-charge generality. ✅ BUILT.** Removed the Safe-v1 restriction so the unified editor collects a price increase for **any** change (cross-week, more children, or added days) via the hosted payment link. The webhook's `handleModificationPaid` now re-prices the **frozen `new_state`** (target week + children + day set) via `computeUnifiedPricing` instead of `computeEditPricing` on the current booking — so cross-week/children up-charges re-price on the right basis, while the drift guard still refunds on a genuine option/discount change. `/edit` drops the `weekChanged||childrenChanged` block and freezes the proposed `club_id`/`num_children` into `new_state`; the booking is untouched until the parent pays. Verified equivalence (same-week add-days re-prices identically to the old path, so no in-flight links wrongly drift-refund). No new migration; webhook left structurally intact (same atomic claim + refund-on-fail). Self-reviewed (adversarial; only a stale doc-comment found + fixed). _Pending: deploy._
- **Cleanup — shared-helper consolidation. ✅ DONE.** Migrated `edit-days` (upcharge + refund) and `reschedule` (refund) off their inline blocks onto the shared `createUpchargeRequest` / `issueRefundAfterModification` helpers — single source of truth for the deferred-upcharge and post-modification-refund flows (~180 lines removed). Behaviour-preserving: identical idempotency keys, Stripe metadata, capacity pre-check, and admin warnings (via the `subject` param). Equivalence-reviewed; one real divergence found + fixed — the refund helper now reports the true refunded amount (not £0) when a post-refund DB write fails after a successful Stripe refund (Phase 5's sync reconciles the ledger), so the customer email is never wrong. No new migration.
