...
- **Phase 7+ (future) — full up-charge generality:** to allow cross-week / children-increase up-charges, generalise the webhook's pending-request re-price (`handleModificationPaid`) to price against the stored `new_state` club/children rather than `computeEditPricing` on the current booking, then drop the Safe-v1 block in `/edit`. Optionally migrate `edit-days`/`reschedule` onto the shared `issueRefundAfterModification`/`createUpchargeRequest` helpers (single source of truth).
